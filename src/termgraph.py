...
def horiz_rows(labels, data, normal_dat, args, colors, legend=[]):
    ...
    for i in range(len(labels)):
        ...
        for j in range(len(values)):
            ...
            if len(legend) == 0:
                ...
                tail = ' {} - {} {}'.format(category_legend[j], values[j],
                                            'occurrences' if values[j] > 1 else 'occurrence')
            if colors:
...
def chart(colors, data, args, labels, legend=None):
    ...
    if not args['stacked']:
        normal_dat = normalize(data, args['width'])
        for row in horiz_rows(labels, data, normal_dat, args, colors, legend):
            if not args['vertical']:
                print_row(*row)
            else:
                vertic = vertically(*row, args=args)
